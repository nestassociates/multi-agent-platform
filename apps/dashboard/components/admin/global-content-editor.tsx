...
import { Loader2, Save, Upload, AlertTriangle, Plus, Trash2 } from 'lucide-react';
import {
  headerContentSchema,
  footerContentSchema,
  legalContentSchema,
...
export function GlobalContentEditor({
  ...
  const isLegalPage = ['privacy_policy', 'terms_of_service', 'cookie_policy'].includes(contentType);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            {isLegalPage
              ? 'Edit the HTML content for this legal page'
      ...
      {isLegalPage ? (
        <LegalPageEditor html={legalHtml} onChange={setLegalHtml} />
      ) : contentType === 'header' ? (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Use the rich text editor to create the legal page content
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="border rounded-lg min-h-[500px]">
          <SimpleEditor initialContent={html} onChange={onChange} />
        </div>
      </CardContent>
    </Card>
  );
