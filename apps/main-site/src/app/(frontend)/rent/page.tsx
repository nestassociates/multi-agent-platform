...
export const metadata: Metadata = {
  title: 'Properties to Rent | Nest Associates',
  description:
    'Browse our selection of properties to rent. Find your perfect rental home with Nest Associates - connecting you with local property experts across the UK.',
  openGraph: {
    title: 'Properties to Rent | Nest Associates',
    description:
      'Browse our selection of properties to rent. Find your perfect rental home with Nest Associates.',
    type: 'website',
  },
}
...
async function PropertyResultsWithCTA({
  ...
  return (
    <>
      ...
      {/* Landlord CTA Banner - contained width */}
      <div className={`container-wide ${secondBatch.length === 0 ? 'pb-12' : ''}`}>
        <CTABanner
          heading="Looking for a letting agent?"
          buttonText="Let Your Property"
          buttonHref="/landlords"
        />
      </div>
      ...
      {result.pagination.totalPages > 1 && (
        <div className="container-wide pb-12 flex items-center justify-center gap-6">
          {page > 1 ? (
            <Link
              href={buildPageUrl('/rent', searchParams, page - 1)}
...
export default async function RentPage({ searchParams }: RentPageProps) {
  ...
  return (
    <div className="bg-[#E8E8E8] bg-[url('/images/wall-shadow-bg.jpg')] bg-cover bg-center bg-fixed">
      {/* Search Bar */}
      <PropertySearchBar transactionType="rental" />
